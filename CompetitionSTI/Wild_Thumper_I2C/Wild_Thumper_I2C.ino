...
void setup()
{
  ...
  pinMode (LED_Serial,OUTPUT);                               // change LED_Battery pin to output
...
void Set_Speed(int Speed, int Steer) {
  ...
  Leftmode=2;
  Rightmode=2;
  if (Leftspeed>0) Leftmode=0;          // if left input is forward then set left mode to forward
  if (Rightspeed>0) Rightmode=0;       // if right input is forward then set right mode to forward

  LeftPWM=abs(Leftspeed)*10/scale;                 // scale 1000-2000uS to 0-255
  LeftPWM=min(LeftPWM,255);                                   // set maximum limit 255

  RightPWM=abs(Rightspeed)*10/scale;              // scale 1000-2000uS to 0-255
...
void RCmode()
{
  ...
  Set_Speed(Speed - 1500, Steer - 1500);
