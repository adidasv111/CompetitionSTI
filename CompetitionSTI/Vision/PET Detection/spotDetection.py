...
def shutdown(channel):  
	os.system("shutdown -h now")	#can add sudo

# ******** GPIO Configuration ********
'''GPIO.setmode(GPIO.BCM)					#pin numbering of the SoC
GPIO.setup(31, GPIO.IN, pull_up_down=GPIO.PUD_UP)		#pulled up. Goes to GND if switch is pressed
GPIO.add_event_detect(31, GPIO.FALLING, callback = shutdown, bouncetime = 2000)   #call shutdown() in case of falling edge in switch
...
# --- Image Processing ---	
	image = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)		#Calculte image in grayscale
